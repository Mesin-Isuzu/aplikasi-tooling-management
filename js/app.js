...
class App {
    ...
    _validateMapUrl(url) {
        if (!url || url.trim() === '') return { valid: true, url: '' };
        const raw = String(url).trim();
        const lower = raw.toLowerCase();
        const isGoogleMaps = lower.includes('google.com/maps') || lower.includes('maps.google.com') ||
            lower.includes('maps.app.goo.gl') || lower.includes('goo.gl/maps');
        if (!isGoogleMaps) {
            return { valid: false, msg: 'URL peta harus berupa link Google Maps, contoh:\nhttps://maps.google.com/?q=Bekasi\natau URL embed: https://www.google.com/maps/embed?pb=...' };
        }

        if (lower.includes('output=embed') || lower.includes('google.com/maps/embed')) {
        ...
        try {
            const u = new URL(raw);
            if (u.searchParams.has('q')) q = u.searchParams.get('q');
        } catch (e) { /* bukan URL absolut */ }

        if (!q) {
            const m = raw.match(/google\.com\/maps\/(place|search)\/([^?&#]+)/i);
            if (m) {
                let path = m[2].replace(/\+/g, ' ').split('/@')[0];
                try { q = decodeURIComponent(path); } catch (e) { q = path; }
            }
        }

        if (q) {
            return { valid: true, url: 'https://maps.google.com/maps?q=' + encodeURIComponent(q) + '&output=embed' };
        }

        if (lower.includes('maps.app.goo.gl') || lower.includes('goo.gl/maps') || lower.includes('google.com/maps/dir')) {
            return { valid: true, url: raw, linkOnly: true };
        }

        return { valid: false, msg: 'URL peta tidak dapat diproses. Gunakan link pencarian Google Maps (mis. https://maps.google.com/?q=Bekasi), URL share, atau URL embed.' };
    }
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div>
                                ...
                                ${t.mapUrl ? ((t.mapUrl.includes('embed') || t.mapUrl.includes('output=embed')) ? `<iframe src="${t.mapUrl}" width="100%" height="120" style="border:0; border-radius: 8px; margin-top: 0.5rem;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>` : `<a href="${t.mapUrl}" target="_blank" rel="noopener" class="btn btn-primary btn-sm" style="margin-top:0.5rem"><i class="fas fa-map-marker-alt"></i> Buka Lokasi di Google Maps</a>`) : '<div style="background: #f1f5f9; padding: 2rem; text-align: center; border-radius: 8px; margin-top: 0.5rem; color: #64748b;">Belum ada titik lokasi</div>'}
                            </div>
                        </div>
                    </div>
                    </div>
